...
from delete_this_trash_pls.foo_anlz.foo_found import calc_a
...
roXF = []
alpha_F = []
for fi in range(1, lenF + 1):

    """импорт множества"""
    path = '/Users/Ivan/Documents/workspace/result/Barrier/XVrange/XVrange%i-%i.csv' % (vi, fi)
    ...
    ro_alpha = mq_value[found_mq_from_alpha(min_cos, vectors_cos)]
    roXF.append(data)
    alpha_F.append(ro_alpha)


psi = psi_foo(roXF, alpha_F)
...
plt.axhline(y=min_cos, xmin=0., xmax=0.99, lw=1, zorder=3, c='g', alpha=0.9)
plt.grid(True)
...
plt.plot([mq_value[found_mq_from_alpha(min_cos, vectors_cos)] for i in range(len(data))], color='g', lw=1.4, alpha=1)
plt.grid(True)
"""
print(psi)
out_psi = psi[np.where(psi > 0.999)]
print('lenX psi > alpha', len(out_psi))
plt.scatter(range(len(psi)), psi, s=0.2)
plt.scatter(np.where(psi > 0.999)[0], out_psi, s=0.3, )
plt.show()
